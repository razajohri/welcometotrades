...
class Glassdoor(Scraper):
    ...
    def scrape(self, scraper_input: ScraperInput) -> JobResponse:
        ...
        # Post-filter: if caller requested remote AND provided a location that looks like Canada,
        # only return jobs marked as remote and whose parsed location references Canada.
        try:
            if (
                scraper_input.is_remote
                and scraper_input.location
                and "canada" in scraper_input.location.lower()
            ):
                filtered = [j for j in job_list if j.is_remote]

                # Also ensure location indicates Canada when available
                def loc_is_canada(j: JobPost) -> bool:
                    if not j.location:
                        return False
                    try:
                        # location can be a dict-like or string; normalize
                        if isinstance(j.location, dict):
                            loc_text = (
                                f"{j.location.get('city','')} {j.location.get('state','')} {j.location.get('country','')}"
                            )
                        else:
                            loc_text = str(j.location)
                        return "canada" in loc_text.lower()
                    except Exception:
                        return False

                filtered = [j for j in filtered if loc_is_canada(j)]
                return JobResponse(jobs=filtered)
        except Exception as e:
            log.debug(f"Glassdoor post-filtering error: {e}")
    ...
    def _get_location(self, location: str, is_remote: bool) -> Tuple[int, str]:
        # If a specific location string was provided, prefer that even when is_remote is True.
        # Only default to the generic "remote" location when no location was provided.
        if not location:
