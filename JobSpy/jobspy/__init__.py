...
from JobSpy.jobspy.greenhouse import Greenhouse
from JobSpy.jobspy.lever import Lever
    ...
    **kwargs,
    ...
    for site, job_response in site_to_jobs_dict.items():
        for job in job_response.jobs:
            ...
            # Add formatted compensation string for frontend display
            min_amt = job_data.get("min_amount")
            max_amt = job_data.get("max_amount")
            interval = job_data.get("interval")
            currency = job_data.get("currency")
            if min_amt and max_amt and interval and currency:
                try:
                    min_amt = int(float(min_amt))
                    max_amt = int(float(max_amt))
                    if min_amt == max_amt:
                        comp_str = f"{currency} ${min_amt:,}/{interval}"
                    else:
                        comp_str = f"{currency} ${min_amt:,} - ${max_amt:,}/{interval}"
                except Exception:
                    comp_str = None
            else:
                comp_str = None
            job_data["compensation"] = comp_str
